...
def update_book_view(request, pk):
    # issue with polymorphisme and select_related
    # https://github.com/django-polymorphic/django-polymorphic/issues/198
    book = Book.objects.get(id=pk) #Book.objects.select_related('product').get(id=pk)
    #product = Product.objects.get(id=book.product)

    f_book        = BookForm(instance=book)
    #print(book.product.pk,flush=True)
    #print(book.pk,flush=True)
    #print(pk,flush=True)

    f_product_select    = SelectProductForm(initial={"product":book.product,"element":book})#BookProductForm(instance=book.product)
    f_product    = BookProductForm(instance=book.product, initial={"element":book})#BookProductForm(instance=book.product)

    if request.method == 'POST':
        if 'save_product_select' in request.POST:
             print(request.POST,flush=True)
             f_product_select = SelectProductForm(request.POST,initial={"product":book.product,"element":book}) #BookProductForm(request.POST, instance=book.product)
             print("Is valid ?",flush=True)
             if f_product_select.is_valid():
                 print("Valid !",flush=True)
                 f_product_select.save()
                 #f_product_select = SelectProductForm(request.POST,initial={"product":book.product.pk,"element":book.pk})
                 book=Book.objects.get(id=pk)
                 f_book = BookForm(instance=book)
                 f_product = BookProductForm(instance=book.product, initial={"element":book})

        if 'save_product' in request.POST:
             #print(request.POST,flush=True)
             f_product = BookProductForm(request.POST,instance=book.product)
             #print(book.product,flush=True)
             if f_product.is_valid():
                 #print(book.product,flush=True)
                 f_product.save()
                 f_product_select = SelectProductForm(initial={"product":f_product.instance,"element":f_book.instance.book})
                 f_book = BookForm(instance=Book.objects.get(id=pk))

        if 'save_book' in request.POST:
            f_book = BookForm(request.POST, instance=book)
            if f_book.is_valid():
               f_book.save()
               f_product_select    = SelectProductForm(initial={"product":f_book.instance.product,"element":f_book.instance})#BookProductForm(instance=book.product)
               f_book = BookForm(instance=f_book.instance)
               f_product = BookProductForm(instance=f_book.instance.product, initial={"element":book})

    context = {
        "f_product_select":f_product_select,
        "f_product":f_product,
        'form': f_book,
    }
